...
def multi_index_to_position(
    ...
    *,
    ...
    if len(shape) == 2:
        ...
        return (i0 * cell, i1 * cell, 0.0)
    if len(shape) == 3:
        i0, i1, i2 = indices[0], indices[1], indices[2]
        return (i0 * cell, i1 * cell, i2 * cell)
    if len(shape) == 4:
        # d0 volumes along +X, each volume is d1 x d2 x d3 in (x,y,z) local space
        s0, s1, s2, s3 = shape
        i0, i1, i2, i3 = indices[0], indices[1], indices[2], indices[3]
        span_x = max(1, s1) * cell
        origin_x = i0 * (span_x + volume_gap)
        return (origin_x + i1 * cell, i2 * cell, i3 * cell)
    ...
    span_y_block = max(1, s1) * (max(1, s3) * cell + volume_gap * 0.5)
    origin_y = i0 * (span_y_block + volume_gap)
    origin_x = i1 * (span_x + volume_gap)
    return (origin_x + i2 * cell, origin_y + i3 * cell, i4 * cell)
